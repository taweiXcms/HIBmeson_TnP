...
//////////////////////////////////////////////////////////
  ...
  const char* outfname;
  if(REAL){
    //infile="/mnt/hadoop/cms/store/user/twang/HI_BfinderNtuple/20140930_PAMuon_HIRun2013_28Sep2013_v1/Bfinder_all_*.root";
    ...
    //outfile="/export/d00/scratch/jwang/tawei/TnPBntuple/TnPBntuple/TnPnt_BoostedMC_20140930_Kp.rootT";
  }
    //test//test//test
    //infile="/mnt/hadoop/cms/store/user/twang/HI_BfinderNtuple/20140930_PAMuon_HIRun2013_28Sep2013_v1/Bfinder_all_100_1_TdZ.root";
//    infile="/net/hisrv0001/home/tawei/HeavyFlavor_20131030/Bfinder/Compare20141014/CMSSW_5_3_20/src/test/Bfinder_all.root";
//    infile="/mnt/hadoop/cms/store/user/twang/HI_BfinderNtuple/BfinderBoostedMC_20141022_hckim-HIJINGemb_inclBtoPsiMuMu/Bfinder_all_999_1_qN5.root";
//    outfile="test.root";
  ...
  if(root->GetEntries() == 0 || hlt->GetEntries() == 0) {
    ...
    for (int muIt1=0;muIt1<MuonInfo_size;muIt1++) {
      ...
      for (int muIt2=muIt1+1;muIt2<MuonInfo_size;muIt2++) {
       ...
       //get the first muon
        ...
        d01[size] = MuonInfo_d0[mu1];
        d02[size] = MuonInfo_d0[mu2];
        dz1[size] = MuonInfo_dz[mu1];
        dz2[size] = MuonInfo_dz[mu2];
        dxyPV1[size] = MuonInfo_dxyPV[mu1];
        dxyPV2[size] = MuonInfo_dxyPV[mu2];
        dzPV1[size] = MuonInfo_dzPV[mu1];
        dzPV2[size] = MuonInfo_dzPV[mu2];
        ...
        id1[size]=1;
  	    if(fabs(MuonInfo_dxyPV[mu1])>=3. || fabs(MuonInfo_dzPV[mu1])>=30.) id1[size]=0;
        ...
        id2[size]=1;
  	    if(fabs(MuonInfo_dxyPV[mu2])>=3. || fabs(MuonInfo_dzPV[mu2])>=30.) id2[size]=0;
		...
		if(!MuonInfo_isStandAloneMuon[mu1]) StandAloneMuon_id1[size]=0;
  	    if(fabs(MuonInfo_StandAloneMuon_dxyPV[mu1])>=3. || fabs(MuonInfo_StandAloneMuon_dzPV[mu1])>=30.) StandAloneMuon_id1[size]=0;
		...
		if(!MuonInfo_isStandAloneMuon[mu2]) StandAloneMuon_id2[size]=0;
  	    if(fabs(MuonInfo_StandAloneMuon_dxyPV[mu2])>=3. || fabs(MuonInfo_StandAloneMuon_dzPV[mu2])>=30.) StandAloneMuon_id2[size]=0;
		...
		//gen==122 is full match, i.e., muon match to gen muon, and decay from same gen mother jpsi
        gen[size]=0;
    	genpt[size]=-1;
        int level1=0;
  	    int level2=0;
